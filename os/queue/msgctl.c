...
int main()
{
   int msqid;
    ...
    (msgctl(msqid,IPC_STAT,&ds));  //assinging data to ds structure
     printf("\n\nBefore sending data\n");
      printf("\nmsqid=%d,\nTime of last msg send =%ld,\nTime of last msg received =%ld,\nTime of last change =%ld,\nCurrent no of bytes=%d,\nno. of msg =%d,\nmax allowed in qeue =%d,\nmsgsnd pid =%d,\nmsgrev pid =%d\n",msqid,ds.msg_stime,ds.msg
        ._rtime,ds.msg_ctime,(int)ds.msg_cbytes,(int)ds.msg_qnum,(int)ds.msg_qbytes,(int)ds.msg_lspid,(int)ds.msg_lrpid);
     ...
     printf("\n\nmsgpool %d,  msgmap %d,  msgmax %d,  msgmnb %d, msgmni %d, msgssz %d, msgtql %d, msgseg %hu \n",info.msgpool,info.msgmap,info.msgmax,info.msgmnb,info.msgmni,info.msgssz,info.msgtql,info.ms
        .gseg);
